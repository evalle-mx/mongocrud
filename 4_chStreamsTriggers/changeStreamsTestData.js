 ...
 */
const { MongoClient } = require('mongodb');//Obtain values from another file
...
async function createListing(client, newListing) {
    ...
    const result = await client.db("sample_airbnb").collection(collName).insertOne(newListing);
...
async function updateListing(client, listingId, updatedListing) {
    ...
    const result = await client.db(database).collection(collName).updateOne({ _id: listingId }, { $set: updatedListing });
...
async function deleteListing(client, listingId) {
    ...
    const result = await client.db(database).collection("listingsAndReviews").deleteOne({ _id: listingId });

    console.log(`${result.deletedCount} document(s) was/were deleted. ID: ${listingId}`);
}